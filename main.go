...
func updateConfig() {
	...
	if err == nil && resp.StatusCode == 200 {
		...
		remote, _ := strconv.ParseFloat(strings.TrimSpace(remoteStr), 64)
		if remote > currentVer {
			green("检测到新版本，开始自更新…")
			var installer string
			switch runtime.GOOS {
			case "windows":
				url := "https://gitee.com/idiomeo/gitsod/raw/master/install/bin/gitsod.msi"
				...
				fmt.Println("安装包下载成功！通过该安装程序进行安装")

				if err := startMSI(tmpPath); err != nil {
					log.Fatalf("启动安装程序失败: %v", err)
				}
				os.Exit(0)
			default:
